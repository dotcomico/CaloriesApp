...
public class ProductExporter {
	...
	private ArrayList<Product> sortProductList(ArrayList<Product> productList) {
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			// סדר כך שאם יש מוצרים עם אותו שם אז הכנס קודם את זה בעל המדד: 100 גרם או 100
			// מל
			for (int j = 0; j < tempList.size(); j++) {
				Product itemJ = tempList.get(j);
				if (itemJ.getName() == null || itemJ.getUnit() == null)
				...
				if (itemJ.getName().equals(itemI.getName()) && !itemJ.getUnit().equals(UNIT_100_GRAM)
	...
	private Product copyItem(Product item) {
		return new Product(item.getItemState(), item.getName(), item.getUnit(), item.getCalorieText(),
				item.getBarcode());
	}
	...
	private String generateExportText(ArrayList<Product> list) {
		...
		for (Product item : list) {
			...
			sb.append("\nSystemProductArr.add(new Product(").append(PRODUCT_STATE_SYSTEM).append(", \"")
					.append(item.getName()).append("\", \"").append(item.getUnit()).append("\", \"")
					.append(item.getCalorieText()).append("\", ").append("\"").append(barcode).append("\"));");
		}
