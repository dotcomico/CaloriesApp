...
/**
 * A custom view that allows the user to select a predefined unit from a
 * Spinner, or enter a custom unit using an EditText field.
 * <p>
 * Typically used when entering product details with flexible units like "gram",
 * "cup", or "custom unit".
 */
...
public class UnitSelectorView extends LinearLayout {
	...
	private void showEditDialog() {
		...
		new AlertDialog.Builder(getContext()).setTitle("מדדים מותאמים אישית").setItems(unitNames, (dialog, which) -> {
	...
	private void showDeleteConfirmation(Unit unit) {
		...
		new AlertDialog.Builder(getContext()).setTitle("מחיקת מדד").setMessage(message)
