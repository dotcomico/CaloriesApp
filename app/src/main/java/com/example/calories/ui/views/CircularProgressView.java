 ...
 * android.graphics.Paint; import android.graphics.RectF; import
 * android.util.AttributeSet; import android.view.View; import
 * androidx.core.content.ContextCompat;
 * 
 * public class CircularProgressView extends View {
 * 
 * private Paint backgroundPaint; private Paint progressPaint; private RectF
 * rectF; private float progress = 0f;
 * 
 ...
 * 
 * public CircularProgressView(Context context, AttributeSet attrs, int
 * defStyleAttr) { super(context, attrs, defStyleAttr); init(); }
 * 
 * private void init() { // צבע רקע (אפור בהיר) backgroundPaint = new Paint();
 * backgroundPaint.setColor(0xFFE8F4FF);
 * backgroundPaint.setStyle(Paint.Style.STROKE);
 * backgroundPaint.setStrokeWidth(20f); backgroundPaint.setAntiAlias(true);
 * 
 * // צבע התקדמות (כחול) progressPaint = new Paint();
 * progressPaint.setColor(0xFF2196F3);
 * progressPaint.setStyle(Paint.Style.STROKE);
 * progressPaint.setStrokeWidth(20f); progressPaint.setAntiAlias(true);
 * progressPaint.setStrokeCap(Paint.Cap.ROUND);
 * 
 * rectF = new RectF(); }
 * 
 ...
 * 
 * int width = getWidth(); int height = getHeight(); int radius =
 * Math.min(width, height) / 2 - 40;
 * 
 * int centerX = width / 2; int centerY = height / 2;
 * 
 * rectF.set(centerX - radius, centerY - radius, centerX + radius, centerY +
 * radius);
 * 
 * // ציור המעגל הרקע canvas.drawCircle(centerX, centerY, radius,
 * backgroundPaint);
 * 
 * // ציור התקדמות float sweepAngle = 360 * progress; canvas.drawArc(rectF, -90,
 * sweepAngle, false, progressPaint); }
 * 
 * public void setProgress(float progress) { this.progress = Math.min(1.0f,
 * Math.max(0.0f, progress)); invalidate(); }
 * 
