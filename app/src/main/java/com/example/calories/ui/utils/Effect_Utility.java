...
public class Effect_Utility {
	...
	// changeImageWithLiveEffect(iv_backToMain,
	// R.drawable.ic_baseline_arrow_circle_right_blue);

	// אהבתי : // changeImageWithBounce , changeImageWithDynamicRotation ,
	// changeImageWithMirrorEffect
	...
	public static void changeImageWithBounce(ImageView imageView, int newImageRes) {
		...
		imageView.animate().scaleX(1.2f).scaleY(1.2f).setDuration(300) // התמונה קופצת
				.withEndAction(() -> imageView.animate().scaleX(1f).scaleY(1f).setDuration(100)); // התמונה מתייצבת
																									// במקום
	}
	...
	public static void crossfadeImage(ImageView imageView, int newImageRes) {
		...
		TransitionDrawable transitionDrawable = new TransitionDrawable(new Drawable[] { imageView.getDrawable(), // התמונה
																													// הנוכחית
